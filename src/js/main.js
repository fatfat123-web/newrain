...
function QingLv(config, game){
    ...
    this.hitted = function(sprite){
        if(Math.random() < 1/4 && ids.length > 0) {
            ...
            //点击获得红包,游戏暂停 这个肯定要去掉 把生成的东西贴到背景图上
            game.paused = true;
...
states.main = function(game) {
    this.create = function() {
        ...
        // 开始游戏倒计时   这里要改成点击后才开始不能是默认开始
        //逻辑要换一下 这里要变成点击后执行这些事件
    ...
    this.startGame = function(daojishi){
        ...
        //添加按钮,并绑定事件 嗯这里要拿去上面用 拿去上面用了之后把这个按钮做成点击的开关
        ...
        // 剩余时间     上面的time变成了30，但是最后5秒的时候没有红包掉下去
